...
export class SavedPropertyService {
  ...
  static async getSavedProperties(userId: string): Promise<ApiResponse<SavedProperty[]>> {
    try {
  ...
  // Separate method for getting saved properties with details
  static async getSavedPropertiesWithDetails(userId: string): Promise<ApiResponse<Array<SavedProperty & { property: Property }>>> {
    try {
      // Get saved properties first
      const { data: savedProperties, error: savedError } = await this.getSavedProperties(userId)
      
      if (savedError || !savedProperties) {
        return { data: null, error: savedError || 'No saved properties found' }
      }

      // Get property details for each saved property
      const propertiesWithDetails = await Promise.all(
        savedProperties.map(async (saved) => {
          const { data: property } = await PropertyService.getProperty(saved.property_id)
          return {
            ...saved,
            property: property!
          }
      ...
      return { data: propertiesWithDetails.filter(item => item.property), error: null }
    } catch (error) {
      console.error('Unexpected error:', error)
      return { data: null, error: 'Unexpected error occurred' }
    }
...
export class InquiryService {
  ...
  static async getInquiriesForAgent(agentId: string): Promise<ApiResponse<Inquiry[]>> {
    try {
